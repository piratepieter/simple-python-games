...
class Vindue(arcade.Window):
    ...
    def on_draw(self):
        ...
        for i in range(len(self.sne_x)):
            self.sne_y[i] -= 10
            arcade.draw_circle_filled(self.sne_x[i], self.sne_y[i], 7, arcade.color.WHITE)

    def on_key_press(self, key, key_modifiers):
        if chr(key) == 's':
            self.sne_x.append(random.randint(0, self.width))
            self.sne_y.append(self.height)
